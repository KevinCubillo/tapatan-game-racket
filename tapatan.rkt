...
; Al ejecutar el programa se desplegará una ventana con el tablero de tapatan y un mensaje que dice "tecla 1: incia el jugador azul, tecla 2: incia el jugador rojo".
; En este punto podemos seleccionar el jugador que va a iniciar. Una vez hecho esto los jugadores puden empezar a poner sus fichas dependiendo del turno, esto hasta que
; alguno alcance formar 3 en linea o hasta que se acaben las fichas, en cuyo caso se podran mover las fichas colocadas a las celdas cercanas siempre que esten vacias,
; buscando formar 3 en linea y de esta forma ganar el juego. Cuando un jugador gana se muestra un mensaje de "!Ganaste jugador rojo!" o "!Ganaste jugador azul" dependiendo del ganador.
; El tablero se bloqueará y para finalizar la ejecuión del programa se deberá cerrar la ventana.
...
; Funcion que muestra el estado del juego. Entradas: ninguna. Salidas: despliegue de la ventana con el tablero. Esta funcion muestra el estado actual
; de la partida, asi como mensajes para los jugadores. Utiliza la variable global initial-board para saber las posiciones de las fichas en el tablero.
(define (draw-board a)
...
; Funcion que determina la celda en la que se va a colocar una ficha. Entradas coordenas (x,y) capturadas al hacer click. Salida: ninguna(indirectamente modifica el initial-board)
; Comprueba que el click de haya dado en una posicion valida, y de ser asi determina si es posible o no realizar la jugada.
...
;Funcion que reemplaza un elemento de una lista. Entradas: una lista, un indice y el valor a setear. Salida: la lista modificada. 
...
;Funcion auxiliar de move, la cual determina las posiciones validas de movimiento. Entrada: El indice de la posicion. Salida: una lista con las celdas adyacentes.
...
;Funcion que mueve una ficha de una poscion a otra. Entradas: La poscion actual, la posicion a desplazarse, el jugador actual. Salidas ninguna(modifica el initial-board).
...
;Funcion que coloca una ficha en una celda . Entradas: la poscion a colocar la ficha y el jugador actual. Salidas ninguna(modifica el initial-board).
...
;Funcion que determina si una celda está vacia. Entrada: la poscion a comprobar. Salida: True si está vacia y False si no.
(define(isEmpty pos)
    (cond [(equal? (list-ref initial-board pos) "")]
     [else #f]))


;Funcion que cambia el jugador actual. Entrada: ninguna. Salida: ninguna(mofica la variable global PLAYER)
...
;Funcion para verificar si alguien ganó el juego. Entrada: ninguna. Salida: ninguna(modifica la variable global check-winners).
; Busca en todas las posiciones posibles que hayan 3 fichas del mismo color alineadas, si las encuentra actualiza el valor de check-winners.
(define(check-winner)
...
;Funcion que establece el jugador que inicia la partida al presionar la techa 1 o 2. Entradas: la tecla presionada. Salida: ninguna(modifica el valor de la variable global PLAYER, seteando el jugador seleccionado).
...
;Funcion Big-bang. Entradas: ninguna. Salidas: ninguna. Función principal de la biblioteca de graficos para racket big-bang.
 (big-bang 0
